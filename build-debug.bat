@echo off
REM Vector v0.4.9 Nuitka DEBUG build (console enabled for tracebacks)
REM Run from project root with .venv activated.

if exist "Vector-v0.4.9-debug.dist" (
    echo Removing stale Vector-v0.4.9-debug.dist...
    rmdir /s /q "Vector-v0.4.9-debug.dist"
)

python -m nuitka ^
  --standalone ^
  --enable-plugin=pyqt6 ^
  --output-filename="Vector-v0.4.9-debug.exe" ^
