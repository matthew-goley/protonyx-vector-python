...
class ProfilePage(QWidget):
    ...
    def _build_ui(self) -> None:
        ...
        self._logout_button.setMinimumWidth(sc(160))
    ...
    def _build_hero(self) -> QWidget:
        ...
        hero_layout.setContentsMargins(sc(24), sc(24), sc(24), sc(24))
        hero_layout.setSpacing(sc(16))

        avatar = QLabel('\U0001F464')
        avatar.setStyleSheet(f'QLabel {{ font-size: {scpt(42)}pt; }}')
        hero_layout.addWidget(avatar)

        text_container = QVBoxLayout()
        text_container.setContentsMargins(0, 0, 0, 0)
        text_container.setSpacing(sc(2))
        self._account_label.setProperty('role', 'muted')
        self._account_label.setStyleSheet(f'QLabel {{ font-size: {scpt(10)}pt; font-weight: 600; }}')
        self._username_hero.setStyleSheet(f'QLabel {{ font-size: {scpt(24)}pt; font-weight: 700; }}')
        self._member_since_hero.setProperty('role', 'muted')
        self._member_since_hero.setStyleSheet(f'QLabel {{ font-size: {scpt(10)}pt; }}')
        ...
        self.total_value.setStyleSheet(f'QLabel {{ font-size: {scpt(30)}pt; font-weight: 700; }}')
    ...
    def _build_details(self) -> QWidget:
        ...
        details_layout.setContentsMargins(sc(24), sc(8), sc(24), sc(8))
    ...
    def _build_row(label_text: str, value_label: QLabel) -> QWidget:
        ...
        row_layout.setContentsMargins(0, sc(14), 0, sc(14))
        row_layout.setSpacing(sc(16))

        key = QLabel(label_text)
        key.setProperty('role', 'muted')
        key.setStyleSheet(f'QLabel {{ font-size: {scpt(9)}pt; font-weight: 700; }}')
        value_label.setAlignment(Qt.AlignmentFlag.AlignRight | Qt.AlignmentFlag.AlignVCenter)
        if not value_label.styleSheet():
            value_label.setStyleSheet(f'QLabel {{ font-size: {scpt(12)}pt; font-weight: 700; }}')
    ...
    def _populate_user_data(self, user_data: dict | None = None) -> None:
        ...
        if isinstance(plan_raw, str) and plan_raw:
            ...
            if plan_raw.lower() == 'free':
                ...
                self._plan_value.setStyleSheet(f'QLabel {{ font-size: {scpt(12)}pt; font-weight: 700; }}')
            else:
                self._plan_value.setProperty('role', '')
                self._plan_value.setStyleSheet(f'QLabel {{ font-size: {scpt(12)}pt; font-weight: 700; color: #38bdf8; }}')
        else:
