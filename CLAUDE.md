...
| `vector/pages/ticker_detail.py` | `TickerDetailPage`, `_InfoCard`, `_PriceChartCard` - single-ticker detail view (info readout + price chart). Not in the sidebar; reached by clicking a ticker. See **Ticker Detail Page**. |
...
3. `MainShell` hosts an `sc(220)` px sidebar + `QStackedWidget` with `DashboardPage`, `VectorLensPage`, `ProfilePage`, `SettingsPage`, and `TickerDetailPage` (index 4, no sidebar entry, see **Ticker Detail Page**) (all base sizes scaled — see **UI Scaling**). Header contains a "?" icon button (`sc(48)`², rounded) that opens the keyboard-shortcuts modal. On window resize, `self.notifications.reposition_all()` keeps the toast stack pinned to the top-right.
...
### Ticker Detail Page (`pages/ticker_detail.py`)

A single-ticker detail view. **It is NOT in the sidebar**; it lives in the `MainShell` `QStackedWidget` at **index 4** and is reachable only by clicking a ticker. It follows the same scrollable pattern as the other pages (`QScrollArea` with `setWidgetResizable(False)` + `container.setFixedWidth(_CONTENT_W())`).

**Layout** (top to bottom): a left-aligned **"‹ Back"** button, then a header (`scpt(24)` ticker symbol + muted company name), then a two-column body: a fixed `sc(340)` `_InfoCard` on the left (`AlignTop`) and an expanding `_PriceChartCard` on the right (mirrors the Lens caution/projections row arithmetic).

- **`_InfoCard`** (`#cardFrame`) renders label/value rows: price (large) + daily change (`$`/`%`, green/red), shares held, equity, sector, beta, annualized volatility, trend slope %, dividend yield, next earnings date, next ex-dividend date, then cost basis + unrealized P&L (`$`/`%`). Rows whose data is missing are **hidden** (no "N/A"): dividend yield only shows when `> 0`; earnings/ex-dividend only when a future date is found. Volatility/slope prefer the per-position values stamped by `compute_portfolio_analytics` (`position['volatility']`/`['slope_percent']`), falling back to computing from `get_history(ticker, '6mo')`. Cost basis uses `position['price']` (entry price) when present, else the current price (so P&L reads ~0 for positions that never stored an entry price). Next ex-dividend is best-effort: the store does not cache a forward ex-dividend date, so the row usually stays hidden.
- **`_PriceChartCard`** (`#cardFrame`) holds the period selector pill buttons (**1M/3M/6M/1Y/2Y maps to `1mo/3mo/6mo/1y/2y`**, default **6M**, active one painted with the brand gradient) and a lazily-created matplotlib `FigureCanvasQTAgg`. It reuses the Lens-graph conventions: `subplots_adjust(left=0.06, right=0.88, top=0.90, bottom=0.22)`, `setMinimumHeight(sc(320))`, theme-aware colours, the teal (`#2dd4bf`) price line, and `canvas.wheelEvent = lambda e: e.ignore()` so page scrolling works over the chart. The x-axis ticks are hidden because `get_history` returns a plain close-series with no dates. Empty/short history shows a "No data available" placeholder.

**Data sources:** `DataStore` only (no new fetching layer): `get_quote`/`get_meta` (price, change, beta, dividend yield, name, sector), `get_history(ticker, period, refresh_interval)` (chart closes), `get_earnings`/`get_dividends` (date rows), plus `window.positions` (shares, equity, entry price). All sizes go through `sc()`, fonts through `scpt()`.

**Navigation API & flow:** `window.show_ticker_detail(ticker)` (delegates to `MainShell.show_ticker_detail`) records the page the user came from (`MainShell._detail_prev_page`), calls `detail_page.set_ticker(ticker)`, switches the stack to index 4, sets the header/breadcrumb to the ticker, and deselects every sidebar nav button. The Back button calls `MainShell.go_back()`, which calls `set_page(self._detail_prev_page)`. `set_ticker` resets the chart to 6M and renders; calling it with the already-shown ticker is a no-op. `refresh()` (called from `VectorMainWindow.refresh_data` **only when the detail page is the visible page**) re-renders in place, preserving the current period.

**Entry points (clickable tickers):**
1. **`PositionsListWidget` rows**: `_PositionRow.mousePressEvent` calls `window.show_ticker_detail(ticker)` on a plain left-click, but **`event.ignore()`s while the parent widget is in edit/delete mode** so the event bubbles to `VectorWidget` for drag / delete-confirm. The row is constructed with `owner=self` (the widget) so it can read `_edit_mode`/`_delete_mode`/`_window`.
2. **Settings holdings list**: `remove_list.itemDoubleClicked` fires `SettingsPage._on_holding_double_clicked`, which calls `window.show_ticker_detail`. Double-click is used so single-click still selects for the Remove/Edit buttons.
