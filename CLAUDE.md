...
**Buy amount caps:** `_cap_buy_amount()` is applied to every buy-type priority (5, 6, 7, 9) so diversification deltas remain proportional to portfolio size. After `_dedupe_ctas()`, two more passes run: `_drop_tiny_buys()` removes any buy below `max($200, 1% of equity)` (sub-1% "deposit $30 into KO" suggestions are noise), then `_cap_total_buys()` scales **all** remaining buy CTAs down proportionally so their combined dollars stay within a **tier-aware** fraction of total equity (`_MAX_TOTAL_BUY_FRACTION_BY_TIER` = `high 0.35 / regular 0.30 / low 0.20`). Conservative is capped tightest because its sells are mostly suppressed, so an uncapped buy total would otherwise become the entire (large, positive) net CTA delta — the "deposit tens of thousands into a burning portfolio" case.
...
**Caution score:** 1–99, the **greater** of (a) a trade-flow score (`(Σ sell$ + 0.30·Σ buy$) / total equity × 100`) and (b) an **exposure-weighted risk floor** (`_risk_floor`). The floor is `max` of three continuous components, each mapping analyzer severities through `_SEVERITY_CAUTION_POINTS`: `pos_pts` (weight-averaged per-position danger), `single_pts` (the worst single position, **damped by its weight** `× min(1, w/0.45)`), and `port_pts` (sector over-concentration, plus aggregate volatility/beta only once *elevated* to high/critical — a ~1.0 beta is normal market exposure, not caution). The floor exists so a genuinely dangerous book still scores high in tiers that suppress sells. **Don't regress to the old flat max-of-severities:** it counted the worst single ticker at full weight, so one 41%-vol satellite at 20% weight pinned a "near-perfect" portfolio to 60/99 and the score collapsed into ~5 buckets (8/30/60/88). The weight-aware floor restores granularity (healthy ≈ single digits, disasters = 88) and only lets a *large* dangerous position drive the score. See `_compute_caution_score` / `_risk_floor` in `lens_output.py`.
