...
**Buy amount caps:** `_cap_buy_amount()` is applied to every buy-type priority (5, 6, 7, 9) so diversification deltas remain proportional to portfolio size. After `_dedupe_ctas()`, two more passes run: `_drop_tiny_buys()` removes any buy below `max($200, 1% of equity)` (sub-1% "deposit $30 into KO" suggestions are noise), then `_cap_total_buys(ctas, equity, max_fraction)` scales **all** remaining buy CTAs down proportionally so their combined dollars stay within `max_fraction` of total equity (a fraction of 0 drops every buy).

**Danger-aware buy budget (don't regress):** the `max_fraction` passed to `_cap_total_buys` is **tier-aware AND danger-aware**: `base = _MAX_TOTAL_BUY_FRACTION_BY_TIER[tier]` (`high 0.35 / regular 0.30 / low 0.20`), then `max_fraction = base × (1 − critical_weight)`. `critical_weight` (`_critical_weight` in `cta_engine.py`) is the share of the book held in positions carrying ANY critical signal (volatility / performance / slope / concentration). A book wholly in crisis scales its buy budget to ~0, so the net CTA delta goes **negative** (trim/sell) instead of recommending a large deposit — this is what killed the "deposit $13,800 into a leveraged-ETF fire" case. `critical_weight` is 0 for any healthy/mild book, so normal dilution-buy advice (e.g. a 35% single-stock tilt) is unaffected.
...
**Sell gates / dead weight (don't regress):** Priority 8 (dead weight) is a *cleanup* suggestion — sell a sub-2% odd-lot — so it is **exempt from the `_sell_too_small` / `_MIN_POSITION_VALUE_FOR_SELL` ($1,000) floors**. Those floors would otherwise reject every sub-2% position, which made the priority dead code (zero `dead_weight` CTAs across 150 test runs). It uses its own tiny floor `_MIN_DEAD_WEIGHT_VALUE` ($25) to skip literal penny stubs. Still suppressed entirely for the conservative tier.
