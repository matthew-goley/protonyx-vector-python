...
**Trim proceeds aren't topped up (don't regress):** after the danger-budget cap, if **any `rebalance` (trim) CTA is present**, `compute_ctas` caps the combined buys to the total trimmed (`_cap_total_buys(ctas, equity, rebalance_total/equity)` + a final `_drop_tiny_buys`). A "you're too concentrated" plan should **redistribute** the trim, not be a net deposit — without this the stacked sector buys (esp. aggressive) turned a trim into a net-positive deposit (NVDA-60% aggressive netted +$2,620; now ≤ 0).
