...
from ..constants import INDEX_ETFS, LOW_BETA_BY_SECTOR, SECTOR_SUGGESTIONS
...
def compute_ctas(pool_results: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    def _market_cap(ticker: str) -> float:
        """Return market cap in USD, or 0.0 if not known."""
        if not store:
            return 0.0
        try:
...
def _dedupe_ctas(cta_list: list[dict]) -> list[dict]:
    """Remove duplicate CTAs, resolve action conflicts, cap per-sector buys."""
