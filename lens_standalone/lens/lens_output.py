...
def build_lens_output(
    ...
    ``progress_cb`` (optional) is forwarded to the analysis pool — the
    standalone CLI runner uses it to display per-analyzer timing. The
    Vector app calls without it, so behavior matches the original.

    History persistence (`save_history=True`) writes to the Vector app's
    user data dir via ``vector.paths.user_file``. The standalone CLI runs
    with ``save_history=False`` to avoid that dependency.
    """
    if not positions:
        return {
            'brief': (
    ...
    try:
        pool_results = run_analysis(positions, store, settings, progress_cb=progress_cb)
    except Exception:
...
def _save_snapshot(result: dict[str, Any]) -> None:
    ...
    Imported lazily so the standalone build doesn't drag in vector.paths.
    Only invoked when ``save_history=True`` and the Vector app is present.
    """
    import json
    from datetime import datetime
    from vector.paths import user_file  # type: ignore[import-not-found]
...
def _apply_all_ctas(
    ...
    """Apply every CTA to a copy of positions; return (projected, net_delta)."""
